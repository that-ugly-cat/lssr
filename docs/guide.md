...
- **The earmark ⚑** is the margin of the shared copy: a dot at the head of a record, one per reviewer, with an optional one-line note. Everyone in the review sees everyone's; yours is the only one you can write. It hangs off the *record*, not off a stage, so a note made while reading a title is still there when the full text is read — the same mark shows on Screening and on Assessment. **It counts nowhere**: no PRISMA number moves, no decision changes, it is in no export. The `⚑ earmarked` filter lists your own marks and only ever your own.
