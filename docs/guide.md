...
Criteria can be **edited in place**, and the numbering shown beside them is renumbered after every add, edit and delete so it always matches the list. Both matter more than they look: reviewers cite criteria **by number** in the reason they attach to a vote, and those reasons outlive the screening and go into the export. Rewording a criterion therefore does not renumber the others, and deleting one does not leave a gap that the next addition silently reuses. What it does do is change the wording that already-cast votes were argued from — see *Good practices* on re-reading the records a reformulation was meant to reach.
...
**Delete** at this stage is a hard delete and leaves no trace, single or batched. Removals are tracked from screening onward, as exclude decisions with a reason; before screening there is nothing to account for. Both deletes now ask the same way — a bar at the bottom of the page with *Cancel* and *Delete*, rather than the browser's dialog for one record and the bar for many — and both offer **Undo** afterwards.

The undo does not make the deletion any softer: the rows really are gone, and what comes back comes back from a snapshot written to disk the moment before. It restores the **original record ids**, which matters more than it sounds — a record's id is the filename of its stored PDF and half of every "not a duplicate" ruling, so a record returning under a fresh id would return with its full text orphaned. Where an id has been taken in the meantime the record comes back under a new one and the notice says how many. The snapshot is kept for a day, so a notice that faded is not a record lost; and because it is a file on the volume rather than something held in memory, it survives a restart. Tables show the first 500 matches — narrow the filter to see more.

**Adding, editing and deleting do not reload the page** either: the modal closes, the pool count and the rows redraw where they are, and the ticks you had put in the checkboxes survive. Importing is the deliberate exception — it rewrites the whole pool, and nobody is working row by row while it happens.
...
Two failure modes land in `maybe` as well: an answer that will not parse, and a record whose API call failed after its retries. Both now say so, in a reason beginning `screening error:` that names the cause. The `maybe` bucket is therefore not purely the model's uncertainty — read the reasons before drawing conclusions from its size, and **re-run anything whose reason begins that way, because it was never judged**.

That last sentence exists because of a defect that ran for months without being visible. An unparsed reply was parked as `maybe` with an *empty* reason, which is the right decision — fail towards a human — presented in the one way a human cannot act on: indistinguishable from genuine uncertainty. On a live review, 66 of 100 `maybe` records turned out to be unparsed replies, and the pattern showed up only when someone noticed that empty reasons occurred on `maybe` and never on `include` or `exclude`. The cause was the token cap truncating the JSON, which has no closing brace when cut off, so the whole reply was discarded. Raising the cap rescued 60 of the 66; the remainder needed it raised again. If you inherit a corpus screened before this was fixed, the query for the damage is *records whose model reason is empty*.

- **Run on N pending** screens only records with no decision at all. **Re-run** additionally re-does the model's own past calls, and never a human's: a record any reviewer or adjudicator has voted on is theirs. Both counts, and the count of records a human has voted on beside them, are read off the votes themselves rather than off the resolved decision — which matters on a two-reviewer review, where a record with one human vote still reports the model as having decided it.
- **🌓 Dry run** is the deliberate exception, and it is off unless a review switches it on in Settings. It screens *exactly* the records reviewers have already voted on, under the criteria as they stand now, and records the model's answers **beside** theirs: nothing is decided, no PRISMA count moves, no record becomes divergent. It answers one question — *would the model agree with us today?* — and the agreement matrix appears above the record table, with a **🌓 dry run differs** filter for walking the disagreements one by one, the model's reason beside the reviewer's.
- A re-run cannot answer that question, and the reason is worth knowing before you reach for one. While a lone human vote sits below the required number of reviewers, the record's published decision is still the model's; re-running the model over those records would overwrite the very answer you were checking. The dry run exists because hiding that overwrite behind a button would have been the easy thing to build.
- Switching the dry run off in Settings hides the button, the matrix and the filter. It does not delete anything: switch it back on and the same numbers return.
- Each estimate under the buttons is a rough upper bound in one direction only — four characters to the token, ignoring prompt caching. It is an estimate of the *input*: on a real run of 275 records it quoted $1.50 against $2.10 actually spent, because the answers were longer than assumed. Read it as an order of magnitude, not a budget.
- **Alt+click a vote button to say why.** A plain click votes and leaves any reason you wrote before untouched; Alt+click opens a box first and the vote is cast when you save it — Enter saves, Shift+Enter breaks the line, Esc closes without voting. Clearing the box and saving clears the reason, which is the only way to clear one. Until this existed the reason field at screening 1 was reachable only through MCP, and it shows: on one real review, 708 of the 710 human reasons were written from a chat and 2 from the app. A reason travels into the conflict, into the export and into whatever the next reader makes of the record, so it is worth the two extra seconds on the ones that are not obvious from the abstract.
- **Every reason is shown with a name on it**, under the record: `model:` first, then each reviewer. The model's is always visible — it is a pre-screener, not a blinded reviewer, and its decision is on the row anyway. A person's reason follows the same rule as their vote, and is not in the page at all until then, which matters more for the argument than for the verdict it supports. Each is clamped to two lines and opens on a click, because these run long: the median on one real review is 664 characters.
- **Voting is blind.** You cast include / maybe / exclude on the right; every voice on the record — the model, each reviewer by name, the adjudicator, a dry run if there is one — is listed under the decision on the left, and is revealed to you once you have voted, once an adjudicator has closed the record, or if you are the owner. Until then the cell reads *vote to reveal*. `✕` retracts your own vote.
- **Where you were** is kept for you on all four record tables. The last row you acted on — a vote, an edit, an upload, a saved review, an earmark — keeps a lit left edge, and a `↩ #3216` chip beside the filters takes you back to it. It lives in your browser and nowhere else: nobody sees anybody else's, and on a different machine it starts empty rather than pointing somewhere wrong. When that record is not in the filter you are looking at, the chip says so instead of scrolling nowhere.
- **A vote does not reload the page.** The row, the counts and the run buttons redraw in place, a line at the bottom confirms what was recorded — and says when the record did not move the way the vote did, because a lone vote below the required number still leaves the model's decision standing — and the page stays exactly where you were reading. Records that no longer match the filter you are on leave the list, as they always did. The same is true of uploading a full text and of saving a review. Without JavaScript every button still works the old way, by reloading.
- **The earmark ⚑** is the margin of the shared copy: a dot at the head of a record, one per reviewer, with an optional note of a line or two — Enter saves it, Shift+Enter breaks the line, and leaving the field saves too. Everyone in the review sees everyone's; yours is the only one you can write. It hangs off the *record*, not off a stage, so a note made while reading a title is still there when the full text is read — the same mark shows on Screening and on Assessment. **It counts nowhere**: no PRISMA number moves, no decision changes, it is in no export. The `⚑ earmarked` filter lists your own marks and only ever your own.
- **The earmark is not blind, and the votes still are.** You cannot see another reviewer's *vote* until you have cast yours; you can see their earmark immediately. That is a deliberate hole, because a margin that arrives after the decision is useless — *the DOI resolves to the wrong article*, *same cohort as #212* are worth having before you vote, not after. The cost has to be said in the same breath: a note reading "I'd exclude this" is a vote in everything but its name, and reading one before you vote means you did not read that record independently. The tool does not police it. Use the margin for what a colleague could not otherwise know, and put judgement in the vote, where it is counted and attributed.
...
**Upload** takes PDF, DOCX, Markdown or TXT for anything the ladder could not reach. A PDF is stored for the convert pass; the text formats already are the full text and go straight to converted. Your upload clears the retrieval notes on that record, since you chose the file. It does not reload the page either: the row, the four counts and both run buttons redraw where they are, and the confirmation says whether what you attached is readable already or still needs converting.
...
Screening 2 is multi-reviewer with the same resolution order and the same owner adjudication as screening 1, using its own reviewer count. **It is not blind**, unlike screening 1: the review modal lists the other reviewers' decisions whether or not you have made yours. That is how the code has always behaved — this guide claimed otherwise until it was checked — and it is worth knowing which of the two stages gives you an independent second reading and which does not.

**The earmark ⚑ is the same one as on Screening** — same dot, same note, same record — because it hangs off the paper and not off the step. In the modal it sits beside the title, so a thought had while reading the full text can be written without closing anything, and the other reviewers' marks are listed under it.

**Saving closes the modal and leaves the list where it was** — the filter, the search, the sort and the scroll position. This is the page where that mattered most: the save used to return you to the *unfiltered* top of the list, so working through a filtered pile meant re-filtering and re-scrolling after every single record. The confirmation reports what was saved, including a review that extracted fields without recording a decision, which is a legitimate thing to do and used to look identical to one that did.
...
A scoping review is something you ask questions about far more often than you edit: how many records are still unscreened, what the inclusion criteria actually say, which included studies came out of Wales, what the extraction says about study design. Those questions arrive in a conversation. LSSR exposes the same corpus there, through fourteen tools over MCP — twelve that read it, one that votes and one that writes a note in the margin.

**Getting a key.** Profile → MCP keys → New key. Leave *may cast screening votes* unticked for a key that only reads, which is the right default and what every key minted before this existed still is. Paste the endpoint (`https://lssr.borant.eu/mcp`) and the key into your client as an `X-API-Key` header. Clients that cannot set headers can use `https://lssr.borant.eu/mcp/k/<your-key>` instead — there the URL *is* the credential and can end up in access logs, so use one key per client and revoke rather than share.

**Two tools write, the other twelve read.** `vote_screen1` casts or retracts a single title-and-abstract vote, `include`, `exclude`, `maybe` or `clear`, one record per call. `set_earmark` puts your earmark on a record, changes its note or takes it off — also one record per call, and it decides nothing: no PRISMA number moves, no export column appears. It is the right home for what a vote's reason keeps being asked to carry, like *this is the same study as 3286 under another DOI* or *the abstract on this record belongs to a different paper*; a verdict is not that, and belongs in the vote. Note that the call describes the end state, so omitting `note` on a record you have already annotated clears the note rather than leaving it. Everything else is still off the surface. No extraction, no adjudication, no screening 2, no step marked done, and no duplicating or deleting a review — a v2 is created by a person, in the UI.

**Writing belongs to the key, not to you.** Ticking the box at mint time is what makes a key able to vote, and there is no route that turns it on afterwards: a capability that can be added later is one that can be added to a key already sitting in a config file somewhere. So a key that only reads still exposes a corpus and cannot corrupt one, and a key that votes can damage one stage of one review and nothing beyond it. To change your mind, revoke and mint another.

**The vote carries your name.** It is an ordinary reviewer row, indistinguishable from one cast in the browser and resolved by the same rules — an adjudicator outranks it, two humans differing make a conflict, and until the review's required number of reviewers have voted the model's provisional decision still stands. Its reason always ends in `[via MCP]`, so you can see in the screening table and in the export which votes came from a conversation. Casting a vote and settling a record are separate events, and the tool reports both.
...
- A dry run's votes appear here too, labelled *the model, dry run (does not count)*. They are not a second opinion and must not be totalled with the rest: they decide nothing, and the `divergent` filter leaves them out on purpose.
- **Every vote is visible here, even the ones the UI hides.** Blinding is a discipline of the *moment of voting*, which happens in the app; this surface exists to read a corpus, and a reader that saw half the votes would mostly produce wrong totals. The cost has to be said plainly: a reviewer who reads here before voting there has read ahead, and an assistant that reads a record and then votes on it has read every other voice first, the model screening pass included. What it casts is therefore not an independent second reading and should not be counted as one — use it to work a pile against the criteria, and adjudicate in the app, where the blinding is.
...
- **Write the criteria for the model that will apply them, not only for the reviewers who wrote them.** A criterion that carves out an exception in its second sentence is read as its first sentence. On one review, an exclusion criterion for attitude surveys "with no reform at issue" carried an explicit exception — a study in a jurisdiction that has already transitioned is not excluded, even where it reports no outcome — and the pre-screener applied the rule while ignoring the exception, excluding population studies from Spain, Portugal and Singapore that were among the most relevant records in the pool. Reading a sample of the model's *exclusions* against the criterion text, not just its inclusions, is what catches this; the ratio worth watching is how many of its exclusions a reviewer overturns, and where.
- Every reviewer's **earmark** on a record comes back from `get_record`, theirs and yours, each with its note and a `mine` flag. `search_records(earmarked=True)` narrows to your own — what other people have flagged is a different question and not one that filter answers. Marks are read here and written in the app.
- **Reformulating a criterion mid-review leaves stale votes behind.** A vote argued from the old wording does not change when the wording does, and it will not look stale — it will look like a decision, with a reason attached. After a reformulation, go back to the records it was written to reach and re-read them: on one real review a single rewording of "no data" to "no data and no developed argument" moved 172 of 435 previously-excluded records back into contention, and left twelve human exclusions citing a criterion that no longer said what they said it said. The **dry run** is one way to find them, since it re-screens exactly the voted records under today's criteria and tells you where the model now differs.
- **Write down decisions that are not criteria.** Excluding congress abstracts, student theses, editorials or a container record for a book whose chapters you already have are all legitimate reviewer decisions, and none of them is an exclusion criterion. Say so in the reason — *"excluded as a conference abstract, by the reviewers' decision of DD Month, not on any of the four criteria"* — because the reason field is the only place that distinction survives into the export, and a year later a record-type rule applied silently is indistinguishable from a criterion applied badly.
