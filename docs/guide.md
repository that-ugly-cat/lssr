...
A scoping review is something you ask questions about far more often than you edit: how many records are still unscreened, what the inclusion criteria actually say, which included studies came out of Wales, what the extraction says about study design. Those questions arrive in a conversation. LSSR exposes the same corpus there, through thirteen tools over MCP — twelve that read it and one that votes.
...
**One tool writes, the other twelve read.** That tool is `vote_screen1`: it casts or retracts a single title-and-abstract vote, `include`, `exclude`, `maybe` or `clear`, one record per call. Everything else is still off the surface. No extraction, no adjudication, no screening 2, no step marked done, and no duplicating or deleting a review — a v2 is created by a person, in the UI.
